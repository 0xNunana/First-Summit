  ...
  return (
    <div className="border bg-white border-gray-300 rounded-lg shadow-md group relative">
      <Image
        ...
        height={200}
        width={500}
        className="w-full object-cover rounded-t-lg h-[150px] md:h-[250px] group-hover:opacity-80 group-hover:cursor-pointer"
      />
      <div className="p-4">
        <p className="font-bold md:text-xl">{item.title}</p>

        <ul className=" bg-white p-2 rounded-t-lg list-inside list-disc">
          <p className="font-semibold text-sm md:text-lg text-black/90">
            Job Opportunities
          </p>
      ...
      <ul className="hidden list-inside group-hover:block absolute bottom-0 bg-white px-5 py-2 rounded-t-lg list-disc w-full cursor-pointer">
        {item.details.map((detail, i) => (
          <div key={i} className="md:p-2 lg:p-4">
            <p className="max-sm:text-sm font-bold">{detail.title}:</p>
            <li className="max-sm:text-xs text-gray-700">
              {detail.description}
            </li>
          </div>
        ))}
